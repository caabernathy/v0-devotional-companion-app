-- Seed a sample devotional for the first available user (no-op if no users exist)
DO $$
DECLARE
  demo_user UUID;
BEGIN
  ...
  IF demo_user IS NULL THEN
    RAISE NOTICE 'Skipping devotional seed: no users found.';
    RETURN;
  END IF;

  INSERT INTO public.devotionals (user_id, date, verse_reference, verse_text, reflection, prayer_points, themes)
  VALUES (
    demo_user,
    CURRENT_DATE,
    'Philippians 4:13',
